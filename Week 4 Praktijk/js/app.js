...
function train() {
    ...
    for(let i = 0; i < landmarks.length; i++) {
        for(let j = 0; j < 3; j++) {
            landmarksArray.push(landmarks[i][j])
        }
    }
    
    machine.learn(landmarksArray, name)
    console.log(landmarksArray)
    console.log(name)
}

classifyButton.addEventListener("click", () => prediction())

function prediction() {
    let landmarksArrayClassify = []

    for(let i = 0; i < landmarks.length; i++) {
        for(let j = 0; j < 3; j++) {
            landmarksArrayClassify.push(landmarks[i][j])
        }
    }
    
    let resultPrediction = machine.classify(landmarksArrayClassify)
    result.innerText = `I think it's a ${resultPrediction}`
    console.log(`I think it's a ${resultPrediction}`)
}
